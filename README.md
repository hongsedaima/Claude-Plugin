...
这个项目的目标不是鼓励你全量安装，而是像一本说明书：基于实际项目、当前任务和风险边界，帮助你按需选择、按需安装、按需开启，并把启停规则沉淀到项目记忆里，让后续 Agent 能主动提醒和执行正确的插件策略。
...
用户只需要告诉 Agent 项目需求、当前进度和关键约束，Agent 就应该基于本指南完成三件事：

1. 判断当前项目需要哪些插件，不需要哪些插件。
2. 规划必要插件的安装、常驻、按需开启和关闭策略。
3. 把这些规则写入 `CLAUDE.md`、`AGENTS.md` 或项目记忆，让后续 Agent 在关键时刻主动提醒，甚至在获得授权后自动执行。

这样，新手不需要记住每个插件的细节，也不需要每次手动判断什么时候开关插件。项目进入新功能开发、前端实现、PR 审查、Bug 修复、上线前检查、网页抓取、GitHub 操作、财务/法律/敏感数据处理等场景时，Agent 会按规则提醒用户安装、开启、停止或谨慎授权对应插件。
...
**新手只需要复制下面这一段给 Claude Code 等 Agent，Agent 会先补齐项目关键信息，再给出必要的插件清单，并把后续启停规则写入项目记忆。**

```text
请帮我为当前项目初始化一套插件管理规则：选择 Claude 官方插件市场中的必要插件，规划安装/启停策略，并把规则沉淀到项目记忆里。
请先阅读这个指南：https://github.com/hongsedaima/Claude-Plugin
然后按下面流程执行：
1. 先检查当前项目是否已有足够信息来判断插件选择。至少需要知道：项目目标、技术栈、语言/框架、包管理器、测试方式、部署方式、当前开发阶段、正在推进的任务、是否有 README/CLAUDE.md/AGENTS.md、是否使用 GitHub/Notion/浏览器自动化/外部网页抓取、是否涉及安全/财务/法律/敏感数据。
2. 能从项目文件中读取的信息请自己读取；如果信息不足，不要直接推荐插件，请先向我提出最少但必要的问题。
3. 信息足够后，按四类输出插件建议：
   - 必装且建议常驻：安装后默认开启，并说明原因。
   - 建议安装但按需开启：说明什么任务或场景下再开启。
   - 暂不建议安装：说明为什么现在不需要。
   - 需要谨慎授权的插件：涉及浏览器、网页抓取、GitHub、外部服务、财务/法律/敏感数据时单独列出。
4. 对每个推荐插件说明：插件名、官方插件页、解决的问题、常驻/按需、典型用法、可能重叠或冲突的插件。
5. 基于本项目情况和指南内容，生成一段“项目插件管理规则”。这段规则要让后续 Agent 能做到：遇到新功能开发、前端页面、PR 审查、修 bug、上线前检查、长期记忆、项目规则维护、网页抓取、GitHub 操作、法律/财务/敏感数据等场景时，主动提醒我安装、开启、关闭或谨慎授权对应插件；如果环境允许且我已授权，可以自动执行必要的安装和基础设置；遇到功能重叠的插件时先说明取舍，不要默认全部开启。
6. 如果当前项目有 CLAUDE.md、AGENTS.md、项目记忆文件或类似长期上下文，请展示你建议写入的目标文件路径和具体内容；等我确认后再合并写入，不要覆盖已有团队约定。如果没有合适文件，先建议创建位置并等待确认。
7. 如果当前环境支持插件安装或配置，请先给出将要执行的命令和影响范围；等我确认后再安装、启用或写配置。如果我已经明确授权你自动执行，则直接完成安装和基础设置，并在最后列出你改了什么。
8. 最后给我一份简短的日常使用规则：哪些插件平时保持开启，哪些插件只在写前端、做 PR、修 bug、上线前检查、整理文档时开启，哪些插件任务结束后要关闭。
